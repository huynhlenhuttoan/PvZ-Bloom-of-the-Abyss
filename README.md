# ─── Plants vs Zombies: Bloom of the Abyss ───

A fan-made, non-profit Pygame project combining the strategic fusion of **PvZ Fusion** with the intense turn-based reaction mechanics of **Clair Obscur: Expedition 33**.

> **Disclaimer:** This game is created strictly for entertainment and coding practice, **not for commercial purposes**. All core concepts and graphics belong to their respective owners.

---

## 💡 Acknowledgments & Credits

* **PvZ Fusion:** Huge thanks to **蓝飘飘fly** for creating this wonderful game and inspiring this project. 
    * [Official PvZ Fusion Bilibili Space](https://space.bilibili.com/3546619314178489/)
* **Clair Obscur: Expedition 33:** Inspiring the unique Turn-based Parry, QTE system and cool SFXs.
* **Plants vs. Zombies**: Featuring amazing background music composed by Laura Shigihara.
* **Plants vs. Zombies: Replanted** Some high-quality assets.
---

## 🎮 Key Features

* **Roguelike Path Selection:** Choose your battle conditions through a dynamic grave map.
* **Active Reaction System:** Command your turn with **QTE Perfect Attacks** and defend with **Timed Parries** to unleash devastating counters.
* **Fusion Mechanics:** Fuse your plants together to discover vastly improved (or weirder) versions of them.
* **Pure Offline Experience:** Single-player gameplay, completely standalone.
* *Status:* **Under Active Development** 🛠️

---

## ⌨️ Controls & Shortcuts

| Key | Action |
| :--- | :--- |
| **Arrow Keys / WASD / Mouse** | Navigating |
| **E / Left-Click**| Confirm selection |
| **Q / Right-Click** | Return to previous selection |
| **E** | Trigger **Parry** (on enemy turn) / **QTE** (on player turn) |
| **Ctrl** | Toggle HP/Armor numeric display mode |

---

## 🚀 Download & Installation

The official release builds will be updated here:
🔗 **[Official GitHub Release Page](https://github.com/huynhlenhuttoan/PvZ-Bloom-of-the-Abyss)**

*No installation required. Just download, extract the ZIP file, and run `PvZ_BotA.exe`!*

---

## 📩 Contact & Bug Report

If you encounter any bugs, crashes, or have any feedback, feel free to reach out via:

* **Facebook:** [facebook.com/toannhut.1984](https://facebook.com/toannhut.1984)
* **Email:** nhuttoan2005@gmail.com
* **Discord:** `cnidalora`

---
